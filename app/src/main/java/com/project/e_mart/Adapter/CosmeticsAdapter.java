...
public class CosmeticsAdapter extends RecyclerView.Adapter<CosmeticsAdapter.Viewholder>{

    ArrayList<PopularDomain> items2;
    private ArrayList<PopularDomain> itemsFiltered;
    Context context;
    public CosmeticsAdapter(ArrayList<PopularDomain> items2) {
        this.items2 = items2;
        this.itemsFiltered = items2;
    }
    public void filter(String query) {
        query = query.toLowerCase();
        itemsFiltered.clear();

        for (PopularDomain item : items2) {
    ...
    @NonNull
    @Override
    public CosmeticsAdapter.Viewholder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull CosmeticsAdapter.Viewholder holder, int position) {
        holder.titleTxt.setText(items2.get(position).getTitle());
        holder.feeTxt.setText("$" + items2.get(position).getPrice());
        holder.score.setText("" + items2.get(position).getScore());
