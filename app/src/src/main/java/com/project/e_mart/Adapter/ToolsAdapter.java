...
public class ToolsAdapter extends RecyclerView.Adapter<ToolsAdapter.Viewholder>{
    ArrayList<PopularDomain> items6;
...
//    }
    public ToolsAdapter(ArrayList<PopularDomain> items6) {
        this.items6 = items6;
        this.itemsFiltered = items6;
    }
    ...
    public void onBindViewHolder(@NonNull ToolsAdapter.Viewholder holder, int position) {
        holder.titleTxt.setText(items6.get(position).getTitle());
        holder.feeTxt.setText("$" + items6.get(position).getPrice());
        holder.score.setText("" + items6.get(position).getScore());

        int drawableResourceId = holder.itemView.getResources().getIdentifier(items6.get(position).getPicUrl(),
        ...
        holder.layoutItem.setOnClickListener(v -> {
            ...
            intent.putExtra("object", items6.get(position));
